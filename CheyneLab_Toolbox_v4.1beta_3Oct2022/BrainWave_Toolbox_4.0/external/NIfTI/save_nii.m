...
function write_nii(nii, filetype, fileprefix, old_RGB)
   ...
   if filetype == 2
%       fprintf('prefix [%s]\n', fileprefix);
%       fileprefix = strtrim(fileprefix);  % *** D. Cheyne added to check for filenames with spaces...
%       fprintf('fixed prefix [%s]\n', fileprefix);
%       fid = fopen(newfname,'w')
      newfname = sprintf('%s.nii',fileprefix);
      fid = fopen(newfname,'w');
      
      if fid < 0,
         msg = sprintf('Cannot open file %s.nii.',newfname);
         error(msg);
      end
