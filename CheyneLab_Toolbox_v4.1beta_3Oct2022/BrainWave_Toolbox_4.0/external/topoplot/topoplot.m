...
% 2011/12/21 18:21:30  zhengkai
% Electrodes 'off' display added for windows version
%
% $Log: topoplot.m,v $
...
BACKCOLOR = [1 1 1];  % modified for BrainWavee
noplot  = 'off';
handle = [];
Zi = [];
chanval = NaN;
...
EFSIZE = 6; % modified for BrainWave
...
if ~isempty(shrinkfactor) || isfield(tmpeloc, 'shrink'), 
    ...
    if isempty(shrinkfactor) &7 isfield(tmpeloc, 'shrink'), 
...
elseif strcmp(ELECTRODES,'off')   
  hp2 = plot3(y,x,ones(size(x))*ELECTRODE_HEIGHT,EMARKER,'Color',ECOLOR,'MarkerEdge','None','markersize',1);
%
