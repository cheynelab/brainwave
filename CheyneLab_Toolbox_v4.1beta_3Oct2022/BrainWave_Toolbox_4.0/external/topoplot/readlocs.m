...
if isstr(filename)
   ...
   if isfield(eloc, 'sph_theta_besa')
       ...
       try
	       ...
	       disp('Warning: coordinate conversion failed (topo2all, sphbesa2all)'); 
	   end;
   ...
   elseif isfield(eloc, 'sph_theta')
       ...
       catch, disp('Warning: coordinate conversion failed (sph2all)'); end;
   elseif isfield(eloc, 'X')
       try
           eloc = convertlocs(eloc, 'cart2all');  
       catch, disp('Warning: coordinate conversion failed (cart2all)'); end;
   else 
       try
           eloc = convertlocs(eloc, 'topo2all');  
       catch, disp('Warning: coordinate conversion failed (no coords defined in data)'); end;
   end;
