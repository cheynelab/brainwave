function [trial latency] = bw_getCTFMarkerLatencies( markerFile, markerName )
%
%   function bw_getCTFMarkers
%
...
% return data for markerName
markerIdx = find(strcmp(names,markerName));

if ~isempty(markerIdx)
    trial = trials{markerIdx}(:,1);
    latency = trials{markerIdx}(:,2);
else
    fprintf('Could not find marker [%s]\n', markerName);
end
