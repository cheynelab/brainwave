...
vertexColors = []; % Anton 2021/05/20 - moved from line 523. Fix for error on line 377
...
FILE_NAME_TEXT=uicontrol('style','text','fontsize',11,'units','normalized','HorizontalAlignment','left',...
    'position',[0.3 0.97 0.6 0.025],'string','','BackgroundColor','white','foregroundcolor','blue','fontweight','b');

SUBJECT_LABEL=uicontrol('style','text','fontsize',11,'units','normalized','HorizontalAlignment','left',...
...
LATENCY_EDIT = uicontrol('style','edit','units', 'normalized',...
    ...
    'FontSize', 12,'callback',@latencyEdit_Callback);
...
MOVIE_RADIO = uicontrol('style','pushbutton','units', 'normalized',...
    ...
    'HorizontalAlignment','left','callback',@movie_Callback);
...
%             {'.txt','*.*';'ASCII Overlay file (*.txt)','All files (*.*)'},...
%             'Select file(s)...');
        [filename, pathname]=uigetfile({'*'},...
                'Select ASCII file...');
...
%             movie2avi(M,movie_name)
            % new Matlab routines to create .avi files
            numFrames = size(FILE_LIST,1);
...
%             set(PLOT_BUTTON,'visible','on');
    ...
    function initMax
        ...
        % reset image num
        IMAGE_NO = 1;
        FILE_LIST = deblank(char(imageset.imageList(IMAGE_NO)));
