...
f=figure('Name', 'BrainWave - Group Image Analysis', 'Position', [scrnsizes(1,4)/6 scrnsizes(1,4)/7  1200 470],...
   ...
   function zmin_callback(src,evt)
    ...
    function make_images(averageOnly)
        ...
        if batchJobs.enabled
            ...
            if ok
                [parentfile, parentpath, ~] = uigetfile({'*VOLUME_IMAGES.mat','VOLUMETRIC IMAGESET (*VOLUME_IMAGES.mat)';
            '*SURFACE_IMAGES.mat','SURFACE IMAGESET (*SURFACE_IMAGES.mat)'; '*.mat','All files (*.mat)'},'Select original group Imageset');
